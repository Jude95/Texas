...
public class Skill {
	...
	public Action getSkillAction(ISceneReader reader) {
		...
		if(canCheck(reader.availableAction())){
			...
			Poker[] pokers = { new Poker(Color.CLUBS, 14),
					new Poker(Color.DIAMONDS, 14) };
			float AAwin = HandStatistics.getInstance().getProbability(pokers,
					reader.person().length);
	...
	public static void main(String[] args) {
		Poker[] poker = { new Poker(Color.HEARTS,2),
				new Poker(Color.CLUBS, 11) };
